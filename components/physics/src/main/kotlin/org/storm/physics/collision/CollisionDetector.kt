...
object CollisionDetector {
  ...
  fun checkMtv(s1: Shape, s2: Shape): Vector {
    ...
    return if (s1.edges.isEmpty() && s2.edges.isEmpty()) calcCircleMtv(s1 as Circle, s2 as Circle) else calcMtv(s1, s2)
  }
  ...
  private fun axes(s1: Shape, s2: Shape): Set<Vector> {
    ...
    if (s1.edges.isEmpty()) {
      axes.add(Vector(s1.center, (s2 as ConvexPolygon).getClosestVertex(s1.center)).counterClockwiseNormal.normalized)
    } else if (s2.edges.isEmpty()) {
      axes.add(Vector(s2.center, (s1 as ConvexPolygon).getClosestVertex(s2.center)).counterClockwiseNormal.normalized)
    }
